  ...
  <div class="home">
    <div class="login_field">
      <el-form :model="user" status-icon :rules="rules" ref="user" label-width="100px" class="demo-ruleForm">
        <h3 v-if="mode == 'code'">
          验证码登录
          <span @click="mode = 'account'; resetForm()"><i class="iconfont icon-phone"></i>账号密码登录</span>
        </h3>
        <h3 v-if="mode == 'account'">
          账号登录
          <span @click="mode = 'code'; resetForm()"><i class="iconfont icon-phone"></i>验证码登录</span>
        </h3>
        ...
        <div v-else class="op_field">
          ...
          <el-form-item label="手机号" prop="phone">
            <div class="top_input">
              <div class="input_area">
                <div class="phone">
                  <el-input type="text" v-model="user.phone" placeholder="请输入您的手机号" />
                </div>
              </div>
            </div>
          </el-form-item>
          <div v-if="mode == 'code'" class="bottom_input">
            <el-form-item label="验证码" prop="code">
              <div class="vcode">
                <el-input type="text" v-model="user.code" placeholder="请输入短信验证码" />
              </div>
              <button @click="getCodeFun()" :disabled="disabled" class="getCode hover">{{ codeTest }}</button>
            </el-form-item>
          </div>
          <div v-if="mode == 'account'" class="bottom_input other_input">
            ...
            <el-form-item label="密码" prop="password">
              <div class="password">
                <el-input type="password" v-model="user.password" autocomplete="off" placeholder="请输入您的密码"
                  show-password />
              </div>
            </el-form-item>
          </div>
          <div :class="{'other_input': mode === 'account', 'loginBtn hover': true }" @click="submit_login_info('user')">
            登录
          </div>
...
export default {
  ...
  methods: {
    ...
    getCodeFun() {
      this.disabled = true;
      this.timer();
      userApi.send_code(this.user.phone, "login")
        .then((res) => {
          this.$message.success(res.message);
